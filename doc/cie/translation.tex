...
\ieclass{y}{\rho}$ implies only $\inot{\inot{\rho(x,y)}}$, rather than
the expected~$\rho(x,y)$. Because this may cause a great deal of
confusion and mistakes, it is better to consider only quotients by
\emph{stable} equivalence relations, which behave as expected.
%
...
$\xall{i}{|a_{i+1} - a_i| < 2^{-i}}$, then the equivalence relation
becomes
%
\begin{equation*}
  \xall{i}{\NN}{|a_i - b_i| < 2^{-i+3}},
\end{equation*}
%
which is stable.
...
Translation of terms. Keep this short.
