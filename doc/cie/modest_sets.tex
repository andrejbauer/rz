...
Alternatively, we could take $\cint$ as the underlying type $\typeOf{Z}$, 
but let $\support{Z}$ include all integers.  In this case, negation and addition could be
simply integer addition and negation\footnote{taking care to avoid integer overflow}.   
Here the neutral element could be implemented as 
any integer multiple of \texttt{7}, and the equivalence $\per_Z$ would be
...
respectively.
\iffalse
\footnote{Incidentally, $\cfun{(k,m)}{(k - m) \cmod
...
presupposition that this equivalence actually be computable (implementable).
...
\internal{Chris}{It seemed out of place to introduce
the idea of subkinds and syntax for $\iStable$ and $\iProp$ here.}
...
We may express uniform families in terms of pers, just like we did for
...
\internal{Chris}{I simplified the discussion of $Z_7$ above, and so we might need to make
changes to the discussion of $Z_n$ to make it fit better.}
