...
\end{figure}
%
A common representation is \textbf{a} pair of lists $(\ell_V, \ell_A)$, where
...
not only what the underlying datatype $\mathtt{graph}$ is, but also
...
store, and other computational effects, provided they behave from outside
as if they did not. A useful example of a functional value using store is
...
We could avoid the overflow
worries by choosing a different, although isomorphic, implementation in which the only valid
realizers are $0$, $1$, \ldots, $6$, and negation and addition are
realized by $\clet{\mathtt{neg}\; k}{7 - k}$ and $\clet{\mathtt{add}\;
  k\; m}{(k + m) \cmod 7}$, respectively.

This particular modest set~$\Z$ also has \emph{decidable equality}, which means
that the characteristic map $\mathrm{eq} : \ZZ_7 \times \ZZ_7 \to
