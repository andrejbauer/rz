...
As a first example we implement the group $(\ZZ_7, 0, {-}, {+})$ of
integers modulo~$7$. We represent the elements of $\ZZ_7 = \set{0, 1,
  \ldots, 6}$ by values of type $\cint$,
%
\begin{equation*}
  \typeOf{\ZZ_7} = \cint.
\end{equation*}
%
An integer value $v \in \values{\cint}$ reprents an element $k \in
\ZZ_7$ if $v \geq 0$ and $v \equiv k \mod 7$.\footnote{The reason for
  requiring $v \geq 0$ is that the Ocaml operator $\mathtt{mod}$
  returns negative remainders when given a negative argument, which is
  something we do not want to worry about.} The associated per is
%
\begin{equation*}
  u \per_{\ZZ_7} v
  \iff
  u \geq 0 \land v \geq 0 \land u \equiv v \mod 7 \;.
\end{equation*}
%
(Mention the word problem and decidable equality at the end.)



Natural numbers. Explicitly postulate the realizer for induction,
using the type $\alpha \to (\alpha \to \alpha) \to \mathtt{nat} \to
\mathtt{\alpha}$ of a realizer for induction.
