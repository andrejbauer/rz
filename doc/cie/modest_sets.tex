...
%
\begin{align*}
  &\clet{\mathtt{neg}\; k}{-(k \;\mathtt{mod}\; 7)} \\
  &\clet{\mathtt{add}\; k\; m}{(k \;\mathtt{mod}\; 7) + (m \;\mathtt{mod}\; 7)}
\end{align*}
%
The expected $\clet{\mathtt{neg}\; k}{-k}$ and $\clet{\mathtt{add}\;
  k\; m}{k+m}$ do not work because of ovwerflow at values that exceed
...
realized by $\clet{\mathtt{neg}\; k}{7 - k}$ and $\clet{\mathtt{add}\;
  k\; m}{(k + m) \;\mathtt{mod}\; 7}$, respectively.)
