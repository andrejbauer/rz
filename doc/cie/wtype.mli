module type Branching =
 sig
   ...
   (**  Assertion symmetric_s =  forall x:s, y:s,  x =s= y -> y =s= x
        ...
        Assertion transitive_s =  forall x:s, y:s, z:s,  x =s= y /\ y =s= z -> x =s= z
   *)
   ...
   (**  Assertion strict_t =  forall x:s, y:t, z:t,  y =(t x)= z -> x : ||s||
        ...
        Assertion symmetric_t =  forall x:s, y:t, z:t,  y =(t x)= z -> z =(t x)= y
         
        Assertion transitive_t = 
          forall x:s, y:t, z:t, w:t,  y =(t x)= z /\ z =(t x)= w -> y =(t x)= w
   *)
...
sig
  ...
  (**  Assertion symmetric_w =  forall x:w, y:w,  x =w= y -> y =w= x
  ...
  *)
  ...
  (**  Assertion 'ty_p [p:w -> ty_p -> bool] induction = 
         ...
         forall f:B.s -> (B.t -> w) -> (B.t -> 'ty_p) -> 'ty_p, 
           (forall (x:||B.s||), 
              forall f':B.t -> w, 
                (forall y:B.t, z:B.t,  y =(B.t x)= z -> f' y =w= f' z) ->
                forall g:B.t -> 'ty_p, 
