...
$\terminating{\typeOf{A}}$ and $\setOf{A}$, satisfying
% 
\begin{enumerate}
...
i.e., symmtric and transitive. Observe that $\support{A} = \set{v \in
  \terminating{\typeOf{A}} \such v \per_A v}$, whence $\per_A$
...
exists $v \in \terminating{\typeOf{A} \to \typeOf{B}}$ for which
%
\begin{equation}
...
$\terminating{\typeOf{A}}$. A morphism $A to B$ is represented by $v
\in \terminating{\typeOf{A} \to \typeOf{B}}$ such that, for all $u, u'
\in \support{A}$,
%
\begin{equation}
...
The category of modest sets, and the equivalent category of pers, have
a very rich structure. For example, the morphisms between modest
sets~$A$ and~$B$ again form a modest set $B^A$, also written as $A \to
B$, called the \emph{exponential} of~$A$ and~$B$, with the underlying
set
%
\begin{equation*}
  \setOf{B^A} =
