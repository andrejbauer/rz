...
(** specifications are expressed in classical logic (negative fragment to be exact) *)
and proposition =
  | True                                       (* truth *)
  | False                                      (* falsehood *)
  ...
  | PLambda of binding * proposition           (* abstraction of a proposition *)
...
(** The substitution functions accept an optional occ argument which is
    ...
    accepts a name (or a string?) and returns a bool. No fresh variable generated by
...
and substProp ?occ sbst = function
  ...
  | PLambda ((n, ty), p) ->
      let n' = freshNm [n] [] ?occ sbst in
	PLambda ((n', substTy ?occ sbst ty), substProp ?occ (insertTermvar sbst n (id n')) p)
