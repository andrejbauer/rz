		...
		L.Theory elems ->
		  begin
			...
			Some (L.DeclSet (_,knd)) -> 
			  ...
			  ResError [E.badModelProjMsg nm2 orig_expr 
				       "Name not projectable"]
		  end
	      | _ -> ResError [E.notWhatsExpectedInMsg expr1 
				  "proper model" orig_expr])
	  [E.notWhatsExpectedInMsg expr1 "proper model" orig_expr]
      end
		   ...
		   begin
		     ...
		     in 
			  match LR.joinTypes cntxt tys with
			      LR.NoBecause rsns -> E.tyGenericErrors rsns
			    | LR.YesIf(ty, reqs) ->
		   ...
		   begin
		     ...
		     in
			  match LR.joinPropTypes cntxt pts with
				...
				ResError rsns
			    | LR.YesIf(pt, reqs) ->
				ResProp(L.PCase (trm1, arms), pt, reqs)
		   end
	       | _::_ ->
		   ResError
...
and annotateTerm' cntxt expr yesFn noRsn = 
  (match annotateExpr cntxt expr with
    ...
    | ResError rsns    -> ResError (noMsg :: rsns)
    | _                -> ResError [noMsg] )
...
and annotateModel' cntxt expr yesFn noMsg =
  (match annotateExpr cntxt expr with
